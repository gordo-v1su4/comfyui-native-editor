use native_decoder::{VideoToolboxDecoder, DecoderConfig, NativeVideoDecoder};
use std::path::Path;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match decoder.decode_frame(1.0) {
        Ok(Some(frame)) => println!("Successfully decoded CPU frame at 1.0s: {}x{}", frame.width, frame.height),
    ...
    match decoder.decode_frame_zero_copy(2.0) {
        Ok(Some(frame)) => println!("Successfully decoded IOSurface frame at 2.0s: {}x{}", frame.width, frame.height),
    ...
    for i in 0..5 {
        ...
        if let Ok(Some(frame)) = decoder.decode_frame(timestamp) {
            println!("CPU frame at {}s: {}x{}", timestamp, frame.width, frame.height);
        }
        if let Ok(Some(frame)) = decoder.decode_frame_zero_copy(timestamp) {
            println!("IOSurface frame at {}s: {}x{}", timestamp, frame.width, frame.height);
        }
    }
