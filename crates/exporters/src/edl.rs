use crate::{AssetInfo, ExportConfig, ExportError, timecode};
use anyhow::Result;
use std::path::Path;
use timeline::{Sequence, ItemKind};
...
fn generate_edl(sequence: &Sequence, assets: &[AssetInfo], config: &ExportConfig) -> Result<String> {
    ...
    for (track_index, track) in sequence.tracks.iter().enumerate() {
        for item in &track.items {
            ...
            let record_in = timecode::frames_to_timecode(
                item.from, 
                sequence.fps, 
                config.timecode_format
            );
...
fn parse_edl(content: &str, config: &ExportConfig) -> Result<(Sequence, Vec<AssetInfo>)> {
    ...
    let mut video_track = timeline::Track { name: "V1".to_string(), items: Vec::new() };
    let mut audio_track = timeline::Track { name: "A1".to_string(), items: Vec::new() };
    ...
    for line in content.lines() {
        ...
        if parts.len() >= 8 {
            ...
            let from = timecode::timecode_to_frames(record_in, sequence.fps, config.timecode_format)?;
            let to = timecode::timecode_to_frames(record_out, sequence.fps, config.timecode_format)?;
            ...
            let asset_info = AssetInfo {
                ...
                audio_channels: if track_type.starts_with('A') { Some(2) } else { None },
                sample_rate: if track_type.starts_with('A') { Some(48000) } else { None },
                timecode: None,
    ...
    let max_end = sequence.tracks.iter()
