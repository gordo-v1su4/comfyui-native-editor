...
async fn import_command(
    ...
    info!("Importing {} files into project: {:?}", files.len(), project_path);
    ...
    let project_id = project_path.file_stem()
        .and_then(|s| s.to_str())
...
async fn convert_command(
    ...
    info!("Converting {:?} to {:?} (format: {})", input, output, output_format);
