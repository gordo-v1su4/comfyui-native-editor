...
pub enum DecodeError {
    #[error("backend not available")] NotAvailable,
    #[error("decode failed: {0}")] Failed(String),
}
...
impl NativeDecoderWrapper {
    ...
    fn get_or_create_decoder(&mut self, path: &Path) -> Result<&mut Box<dyn native_decoder::NativeVideoDecoder + Send>, DecodeError> {
        if self.decoder.is_none() {
            ...
            self.decoder = Some(create_decoder(path, config)
                .map_err(|e| DecodeError::Failed(format!("Failed to create native decoder: {}", e)))?);
        }
...
impl VideoDecoder for NativeDecoderWrapper {
    fn decode_yuv_at(&mut self, path: &Path, t_sec: f64) -> Result<YuvFrame, DecodeError> {
        debug!("NativeDecoderWrapper::decode_yuv_at called for path: {:?}, t_sec: {}", path, t_sec);
        let decoder = self.get_or_create_decoder(path)?;
        let frame = decoder.decode_frame(t_sec)
        ...
        if let Some(frame) = frame {
            ...
            debug!("Successfully decoded frame: {}x{} format: {:?}", frame.width, frame.height, fmt);
            Ok(YuvFrame {
                fmt,
                y: frame.y_plane,
                uv: frame.uv_plane,
...
impl VideoDecoder for FfmpegDecoder {
    fn decode_yuv_at(&mut self, path: &Path, t_sec: f64) -> Result<YuvFrame, DecodeError> {
        // Try P010 first
        if let Some(frame) = ffmpeg_decode(path, t_sec, true) { return Ok(frame); }
        // Fallback to NV12
        if let Some(frame) = ffmpeg_decode(path, t_sec, false) { return Ok(frame); }
