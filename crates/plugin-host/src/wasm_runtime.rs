...
impl WasmRuntime {
    ...
    pub fn execute_plugin(
        ...
        let mut store = Store::new(&self.engine, WasmPluginState {
            context,
            logs: Vec::new(),
        ...
        Ok(PluginResult {
            ...
            error_message: if success { None } else { Some("Plugin returned error code".to_string()) },
        })
    }

    fn define_host_functions(linker: &mut Linker<WasmPluginState>) -> Result<()> {
        // Log function
        linker.func_wrap("env", "log", |mut caller: Caller<'_, WasmPluginState>, ptr: i32, len: i32| -> Result<(), anyhow::Error> {
        ...
        // Get current frame
        linker.func_wrap("env", "get_current_frame", |caller: Caller<'_, WasmPluginState>| {
            caller.data().context.current_frame
        })?;
        ...
        // Get sequence height
        linker.func_wrap("env", "get_height", |caller: Caller<'_, WasmPluginState>| {
            caller.data().context.height as i32
        })?;

        // Memory allocation (simple bump allocator)
        linker.func_wrap("env", "alloc", |mut _caller: Caller<'_, WasmPluginState>, _size: i32| -> i32 {
...
pub mod wasm_helpers {
    ...
    pub fn generate_wasm_plugin_template(plugin_name: &str) -> String {
        format!(r#"//! {plugin_name} - A WASM plugin for Gausian Native Editor
